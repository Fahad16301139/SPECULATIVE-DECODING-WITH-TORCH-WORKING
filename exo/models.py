...
model_cards = {
  ...
  "llama-3.1-8b": {
    ...
    "vocab_size": 128000,  # Standard LLaMA vocabulary
    "speculative_compatible": ["llama-3.2-1b", "llama-3.2-8b", "llama-3.2-3b", "llama-3.1-70b", "llama-3-8b", "llama-3-70b", "llama-3.3-70b"],
  },
  "llama-3.1-70b": {
    "layers": 80,
    "repo": {
    ...
    "vocab_size": 128000,  # Standard LLaMA vocabulary
    "speculative_compatible": ["llama-3.2-1b", "llama-3.2-8b", "llama-3.2-3b", "llama-3.1-8b", "llama-3-8b", "llama-3-70b", "llama-3.3-70b"],
  },
  "llama-3.1-70b-bf16": {
    "layers": 80,
    "repo": {
  ...
  ### tiny llm models - compatible with standard llama vocab
  "tiny-llm": {
    "layers": 8,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "arnir0/Tiny-LLM",
    },
  },
  "tiny-llm-10m": {
    "layers": 8,
  ...
  "vicuna-68m": {
    "layers": 6,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "double7/vicuna-68m",
    },
  },
  "tiny-starcoder": {
    "layers": 6,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "bigcode/tiny_starcoder_py",
    },
  },
  ...
  # phi family - compatible vocabularies
  "phi-3.5-mini": { 
    "layers": 32, 
    "repo": { 
      "MLXDynamicShardInferenceEngine": "mlx-community/Phi-3.5-mini-instruct-4bit",
      "TinygradDynamicShardInferenceEngine": "microsoft/Phi-3.5-mini-instruct",
    }, 
  },
  "phi-4": { 
    "layers": 40, 
    "repo": { 
      ...
      "TinygradDynamicShardInferenceEngine": "microsoft/phi-4",
    }, 
  },
  "phi-3-mini": {
    "layers": 32,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "microsoft/Phi-3-mini-4k-instruct",
    },
  },
  "phi-3-medium": {
    "layers": 40,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "microsoft/Phi-3-medium-4k-instruct",
    },
  },
  # Phi-1.5 and Phi-2 - REAL SPECULATIVE DECODING COMPATIBLE (identical ~50K vocab)
  # Both use WordPiece-like vocab with TinyGrad support
  "phi-1.5": {
    "layers": 24,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "microsoft/phi-1_5",
    },
    "vocab_size": 50304,  # WordPiece-like tokenizer - should be identical
    "speculative_compatible": ["phi-2"],
  },
  "phi-2": {
    "layers": 32,
    "repo": {
      "TinygradDynamicShardInferenceEngine": "microsoft/phi-2",
    },
    "vocab_size": 50304,  # WordPiece-like tokenizer - should be identical  
    "speculative_compatible": ["phi-1.5"],
  },
  # dummy
...
pretty_name = {
  ...
  "tiny-llm-10m": "Tiny LLM 10M",
  "vicuna-68m": "Vicuna 68M",
  "tiny-starcoder": "Tiny StarCoder",
  "gemma2-9b": "Gemma2 9B",
...
def get_speculative_compatible_pairs():
    """Get all compatible model pairs for speculative decoding (identical vocab sizes)"""
    compatible_pairs = []
    
    # Get all models with speculative_compatible field
    for model_id, config in model_cards.items():
        if "speculative_compatible" in config:
            vocab_size = config.get("vocab_size")
            for compatible_model in config["speculative_compatible"]:
                if compatible_model in model_cards:
                    compatible_vocab = model_cards[compatible_model].get("vocab_size")
                    if vocab_size == compatible_vocab:
                        # Add both directions (draft->target and target->draft)
                        compatible_pairs.append((model_id, compatible_model))
    
    return compatible_pairs

def is_speculative_compatible(draft_model: str, target_model: str) -> bool:
    """Check if two models are compatible for speculative decoding"""
    if draft_model not in model_cards or target_model not in model_cards:
        return False
    
    draft_config = model_cards[draft_model]
    target_config = model_cards[target_model]
    
    # Must have identical vocab sizes
    draft_vocab = draft_config.get("vocab_size")
    target_vocab = target_config.get("vocab_size")
    
    if not draft_vocab or not target_vocab:
        return False
    
    return draft_vocab == target_vocab

def get_recommended_speculative_pairs():
    """Get recommended model pairs for speculative decoding"""
    recommendations = [
        # LLaMA 3.2 models (confirmed compatible vocab - 128k tokens)
        ("llama-3.2-1b", "llama-3.2-8b", "✅ BEST: LLaMA 3.2 1B → 8B (confirmed 128k vocab, optimal speedup)"),
        ("llama-3.2-1b", "llama-3.2-3b", "✅ Good: LLaMA 3.2 1B → 3B (confirmed 128k vocab, good speedup)"),
        ("llama-3.2-1b", "llama-3.1-8b", "✅ Cross-gen: LLaMA 3.2 1B → 3.1 8B (128k vocab)"),
        ("llama-3.2-1b", "llama-3.1-70b", "✅ Maximum: LLaMA 3.2 1B → 3.1 70B (128k vocab, max performance)"),
        # Phi models (TinyGrad compatible with ~50K vocab) 
        ("phi-1.5", "phi-2", "⚠️ Experimental: Microsoft Phi models (architecture mismatch in exo)"),
    ]
    
    # Filter to only include available models
    available_recommendations = []
    for draft, target, description in recommendations:
        if draft in model_cards and target in model_cards:
            if is_speculative_compatible(draft, target):
                available_recommendations.append((draft, target, description))
