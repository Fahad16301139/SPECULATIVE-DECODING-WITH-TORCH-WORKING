    ...
    if shard.model_id != 'stable-diffusion-2-1-base':
      ...
      if shard.is_last_layer() and not is_finished:
        if generated_tokens is not None:
          self.buffered_token_output[request_id][0].extend(generated_tokens)
          if DEBUG >= 2: 
            print(f"[{request_id}] Multi-token result: {len(generated_tokens)} tokens, total buffered: {len(self.buffered_token_output[request_id][0])}")
          
          is_finished = (
            any(token == self.inference_engine.tokenizer.eos_token_id for token in generated_tokens) or
            len(self.buffered_token_output[request_id][0]) >= self.max_generate_tokens
          )
